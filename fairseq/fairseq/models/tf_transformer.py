...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        if prev_x is not None:
            tf_ratio = 0.5
            if tf_ratio < 1.0:
                output_tokens = prev_x.max(-1)[1]
                output_tokens[prev_output_tokens==2] = 1
                output_tokens[:,1:] = prev_output_tokens[:,:-1]
                output_tokens[:,0] = 2
                prev_len = prev_output_tokens.size()[1]
                randind = torch.randperm(prev_len)[:int((1-tf_ratio)*prev_len)]
                prev_output_tokens[:,randind] = output_tokens[:,randind]
        ...
        return x, {"attn": [attn], "inner_statens": inner_states}
...
@register_model_architecture("tf_transformer", "tf_transformer_tiny")
...
@register_model_architecture("tf_transformer", "tf_transformer")
...
@register_model_architecture("tf_transformer", "tf_transformer_iwslt_de_en")
...
@register_model_architecture("tf_transformer", "tf_transformer_lgy")
