...
then 
    ...
    --log-interval 20 --tensorboard-logdir log/tf/$preix/$name --restore-file $restore_file --reset-optimizer --sample-method $sample --tf-ratio $tf_ratio --warmup-updates $warmup\
    --save-interval 1  --clip-norm 0.1 --max-epoch $max_epoch --max-tokens $maxtoken > log/out/$preix/$name.out 2>&1 &
    ...
    --save-interval 1 --clip-norm 0.1 --max-epoch $max_epoch   --warmup-updates $warmup --max-tokens $maxtoken> log/out/$preix/$name.out 2>&1 &
