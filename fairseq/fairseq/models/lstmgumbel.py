...
@register_model("lstmgumbel")
class LSTMModelGumbel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        print(args.sample_method, args.tf_ratio)
...
class LSTMDecoderGumbel(FairseqIncrementalDecoder):
    """LSTM decoder."""
...
#         if 1 - metrics.BleuLog.minus_gram * metrics.BleuLog.minus_gram_tem <= 0 :
#             self.tf_ratio = 0
#         else:
#             self.tf_ratio = 1 - metrics.BleuLog.minus_gram * metrics.BleuLog.minus_gram_tem
#         self.tf_cnt += 1
#         if self.tf_cnt > 8e3:
#             if self.tf_ratio > 0:
#                 self.tf_ratio -= 3e-5
#         if self.tf_ratio > 0:
#             self.tf_ratio -= 3e-5
#         if 1e4 < self.tf_cnt < 2e4:
#             self.tf_ratio = 0.25
#         elif self.tf_cnt > 2e4:
#             self.tf_ratio = 0.0
            
#         if 6e3 < self.tf_cnt < 12e3:
#             self.tf_ratio = 0.3
#         elif 12e3 <= self.tf_cnt <= 18e3:
#             self.tf_ratio = 0.15
#         elif self.tf_cnt > 18e3:
#             self.tf_ratio = 0.0
        outs = []
        metrics.log_scalar("tf_ratio", self.tf_ratio, round=5)
        for j in range(seqlen):
            # input feeding: concatenate context vector from previous time step
            if input_feed is not None:  # 前面有encoder
                input = torch.cat((x[j, :, :], input_feed), dim=1)
                # x是输入的embedding: len x batch x hidden, input_feed  batch x hidden
                # x[j, :, :] 当前时刻的输入 batch x hidden 需要改
            else:
                input = x[j]
            ...
            is_not_teacher = random.random() > self.tf_ratio

            out_probs = self.output_layer(out)
            if is_not_teacher and j < seqlen - 1:
                if self.sample_method == 'greedy':
                    top1 = out_probs.max(1)[1]
                    x[j + 1, :, :] = self.embed_tokens.weight[top1]
                elif self.sample_method == 'gumbel':
                    out_probs = F.gumbel_softmax(torch.log_softmax(out_probs,dim=-1), tau=5, dim=-1,hard=True)
                    x[j + 1, :, :] = torch.matmul(out_probs, self.embed_tokens.weight)
                elif self.sample_method == 'topk':
                    out_probs = out_probs+torch.randn_like(out_probs)
#                     out_probs = F.gumbel_softmax(torch.log(torch.softmax(out_probs,dim=-1)), tau=1, dim=-1)
                    topk = out_probs.topk(self.top_k,-1)[0][:,-1:]
                    mask = torch.zeros_like(out_probs)
                    mask[mask==0] = -float('inf')
                    out_probs = torch.where(out_probs>topk, out_probs,mask)
                    out_probs = F.softmax(out_probs,dim=-1)
#                     mask = torch.zeros_like(out_probs).cuda()
#                     mask[mask==0] = float('inf')
#                     out_probs = torch.log(F.gumbel_softmax(out_probs, tau=1, dim=-1))
#                     mask[out_probs.topk(40,-1)[1]] = 1
#                     out_probs = out_probs.mul(mask)
#                     out_probs = F.softmax(out_probs,dim=-1)
                    x[j + 1, :, :] = torch.matmul(out_probs, self.embed_tokens.weight)
                elif self.sample_method == 'topk_mean':
                    out_probs = out_probs+torch.randn_like(out_probs)
                    topk = out_probs.topk(self.top_k,-1)[0][:,-1:]
#                     out_probs = F.gumbel_softmax(torch.log(torch.softmax(out_probs,dim=-1)), tau=1, dim=-1)
                    mask = torch.zeros_like(out_probs)
                    mask[mask==0] = -float('inf')
                    mask1 = 1./topk + torch.randn_like(out_probs)
                    
                    out_probs = torch.where(out_probs>topk,mask1 ,mask)
                    out_probs = F.softmax(out_probs,dim=-1)
#                     mask = torch.zeros_like(out_probs).cuda()
#                     mask[mask==0] = float('inf')
#                     out_probs = torch.log(F.gumbel_softmax(out_probs, tau=1, dim=-1))
...
@register_model_architecture("lstmgumbel", "lstmgumbel")
...
@register_model_architecture("lstmgumbel", "lstm_wiseman_iwslt_de_en_gumbel")
def lstm_wiseman_iwslt_de_en_gumbel(args):
    args.dropout = getattr(args, "dropout", 0.1)
...
# @register_model_architecture("lstm", "mylstm")
