...
#         --max-epoch $max_epoch --no-epoch-checkpoints   --skip-invalid-size-inputs-valid-test  --patience 3
#     ((cuda=cuda+1))         --reset-optimizer --reset-dataloader --reset-meters \         --restore-file $BART_PATH \ --validate-interval-updates 500  --patience 5 
    ...
    last_name=$name
    preix=$preix"_rl_wmtraw"
    LR=3e-5
    loss="reward_cross_entropy" #"ngrambleuloss_nat" # cross_entropy
    model="bart_base" # "bart_base_iter"
    tf_ratio=1.0
    max_epoch=10
    name=${loss:0:2}"_"${model:0:2}"_"$tf_ratio"_"$sym
    ...
    CUDA_VISIBLE_DEVICES=$cuda fairseq-train $data \
        ...
        --save-dir checkpoints/denoising_bart_wmt/$preix/$name \
        --tf-ratio $tf_ratio --tensorboard-logdir log/tf/denoising_bart_wmt/$preix/$name \
        --max-epoch $max_epoch --no-epoch-checkpoints  --patience 3 #--validate-interval-updates 000  # --patience 5
done
...
#         --save-dir checkpoints/denoising_bart_wmt/$preix/$name \
#         --tf-ratio $tf_ratio --tensorboard-logdir log/tf/denoising_bart_wmt/$preix/$name \
#         --max-epoch $max_epoch --no-epoch-checkpoints --ngram 1,2 --validate-interval-updates 500  --patience 5 #--validate-interval-updates 000  # --patience 5
